...
# Agent-specific bot tokens for War Room multi-bot demo
AGENT_TOKENS = {
    "orchestrator": os.environ.get("TG_TOKEN_MAIN", "").strip(),
    "datacollector": os.environ.get("TG_TOKEN_DATA", "").strip(),
    "painanalyzer": os.environ.get("TG_TOKEN_ANALYZER", "").strip(),
    "marketvalidator": os.environ.get("TG_TOKEN_MARKET", "").strip(),
    "competitor": os.environ.get("TG_TOKEN_COMPETITOR", "").strip(),
    "businessdesigner": os.environ.get("TG_TOKEN_BUSINESS", "").strip(),
    "shrimpilot": os.environ.get("TG_SHRIMPILOT_TOKEN", "").strip(),
}

def tg_send(text: str, chat_id: str = "", agent_token: str = "") -> bool:
...
def run_demo(chat_id: str = "") -> str:
    ...
    # Send with delays
    # Map each step to its agent's bot token
    step_tokens = [
        AGENT_TOKENS["orchestrator"],      # Step 1: Orchestrator
        AGENT_TOKENS["datacollector"],      # Step 2: DataCollector
        AGENT_TOKENS["painanalyzer"],       # Step 3: PainAnalyzer
        AGENT_TOKENS["orchestrator"],       # Step 4: MarketValidator (via Orchestrator)
        AGENT_TOKENS["businessdesigner"],   # Step 5: BusinessDesigner
        AGENT_TOKENS["shrimpilot"],         # Step 6: GuardShrimp
        AGENT_TOKENS["shrimpilot"],         # Step 7: CareShrimp
        AGENT_TOKENS["shrimpilot"],         # Step 8: OpsShrimp
        AGENT_TOKENS["orchestrator"],       # Step 9: Summary (Orchestrator)
    ]

    for i, msg in enumerate(steps):
        log.info("Sending step %d/9 via agent bot...", i + 1)
        token = step_tokens[i] if i < len(step_tokens) else ""
        tg_send(msg, target, agent_token=token)
        if i < len(steps) - 1:
