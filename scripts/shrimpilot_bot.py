...
def ops_get_hotspots() -> str:
    ...
    hotspots = sb_query(
        ...
        "&collected_at=gte." + (now_bjt() - timedelta(days=7)).strftime("%Y-%m-%dT00:00:00")
    )
    ...
    if hotspots:
        ...
        lines = [f"📊 *7 日热点监测（真实数据）*", f"采集时间: {now_bjt().strftime('%Y-%m-%d %H:%M')}", ""]
    ...
    elif local_summary and local_summary.get("date") == now_bjt().strftime("%Y-%m-%d"):
...
def ops_status() -> str:
    ...
    if health and health.get("date") == now_bjt().strftime("%Y-%m-%d"):
...
def care_break() -> str:
    ...
    health["work_start"] = now_bjt().strftime("%Y-%m-%dT%H:%M:%S")
    write_memory("health_log.json", health)
...
def analyze_meal_photo(file_id: str, chat_id: str, caption: str = "") -> str:
    """Download photo, analyze with Gemini Flash Vision, log meal, give health-aware advice."""
    if not GEMINI_KEY:
    ...
    # Read current health status for personalized advice
    health = read_memory("health_log.json")
    sleep_hours = health.get("sleep_hours_estimated", 0)
    hrv = health.get("hrv_latest_ms", 0)
    rhr = health.get("resting_hr_bpm", 0)
    hrv_history = health.get("hrv_history", [])
    recent_hrv = [h.get("avg", 0) for h in hrv_history[-7:]] if hrv_history else []
    hrv_7d_avg = sum(recent_hrv) / len(recent_hrv) if recent_hrv else 0
    fatigue = health.get("fatigue_level", "unknown")
    burnout = health.get("burnout_signals", 0)
    sleep_data = health.get("sleep_data", {})
    deep_pct = sleep_data.get("deep_pct", 0)

    health_context = (
        f"用户当前健康状态:\n"
        f"- 昨晚睡眠: {sleep_hours}h, 深度睡眠 {deep_pct}%\n"
        f"- HRV: {hrv}ms (7日均值 {hrv_7d_avg:.0f}ms, {'偏低需恢复' if hrv_7d_avg < 35 else '正常'})\n"
        f"- 静息心率: {rhr}bpm\n"
        f"- 疲劳等级: {fatigue}, 倦怠信号: {burnout}/7\n"
    )

    prompt_text = (
        "你是健康虾(CareShrimp)，一个专业的饮食+健康分析助手。用户发送食物照片，你需要:\n"
        "1. 识别菜品并估算营养\n"
        "2. 结合用户当前健康状态，给出个性化评价\n\n"
        f"{health_context}\n"
        "用 JSON 格式返回（不要 Markdown code block，直接返回纯 JSON）:\n"
        '{"dishes": [{"name": "菜名", "cal_est": 300, "protein_g": 15, "note": "简短评价"}], '
        '"total_cal": 800, "health_rating": 4, "summary": "一句话总评", '
        '"improvement": "一个基于用户健康状态的具体改善建议", '
        '"health_match": "这顿饭对用户当前身体状态的匹配度评价(2-3句话，结合睡眠/HRV/疲劳数据)"}\n'
        "health_rating: 1-5 分（1=不健康, 5=非常健康，要考虑用户当前身体状态）\n"
        "如果照片不是食物，返回 {\"error\": \"这不是食物照片\"}\n\n"
    )
    if caption:
        prompt_text += f"补充说明: {caption}\n\n"
    prompt_text += "请识别这张食物照片中的菜品，结合我的健康状态给出分析。"
    ...
    # Parse JSON response
    try:
        # Strip possible markdown code block and extract JSON
        ...
        log.warning(f"Could not parse JSON: {raw_text[:300]}")
        return f"菜品分析:\n\n{raw_text}"
    ...
    # Health-aware personalized advice
    if result.get("health_match"):
        lines.append("")
        lines.append(f"*结合你的健康状态:*")
        lines.append(f"  {result['health_match']}")
...
def reset_decision_chain() -> str:
    ...
    if chain and chain.get("active"):
        chain["active"] = False
        chain["reset_at"] = now_bjt().strftime("%Y-%m-%dT%H:%M:%S")
        write_memory("decision_chain.json", chain)
...
def cron_health_check():
    ...
    health = read_memory("health_log.json")
    if not health or health.get("date") != now_bjt().strftime("%Y-%m-%d"):
