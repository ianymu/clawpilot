...
def fetch_pain_top5() -> list[dict]:
    ...
    # Split into latest-day vs 7-day (find most recent day with data)
    ...
    for p in all_7d:
        ...
        day = cat[:10]
        day_buckets[day].append((lbl, sc))

    # Split 7-day window into recent-half vs older-half for trend
    sorted_days = sorted(day_buckets.keys())
    mid = len(sorted_days) // 2
    recent_days = set(sorted_days[mid:])  # newer half
    older_days = set(sorted_days[:mid])   # older half

    recent_agg = defaultdict(list)
    older_agg = defaultdict(list)
    for day, items in day_buckets.items():
        for lbl, sc in items:
            if day in recent_days:
                recent_agg[lbl].append(sc)
            else:
                older_agg[lbl].append(sc)

    week_avg = {k: round(sum(v)/len(v), 1) for k, v in week_agg.items() if v}
    recent_avg = {k: round(sum(v)/len(v), 1) for k, v in recent_agg.items() if v}
    older_avg = {k: round(sum(v)/len(v), 1) for k, v in older_agg.items() if v}
    # delta = recent half avg - older half avg (shows trend)
    yesterday_avg = {}
    for k in week_avg:
        r = recent_avg.get(k)
        o = older_avg.get(k)
        if r is not None and o is not None:
            yesterday_avg[k] = round(r - o + week_avg[k], 1)  # shift so delta is meaningful
        elif r is not None:
            yesterday_avg[k] = round(r, 1)  # only in recent = new trend
        else:
            yesterday_avg[k] = round(week_avg[k], 1)  # no recent data
